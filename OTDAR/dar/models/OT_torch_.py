...
def split_pos_neg(aa, gap=2):  # 按照下标分割正负文档
    A, B = [], []
    bs = aa.size(0)
    for i in range(0, bs, gap):
        A.append(aa[i])
        B.append(aa[i+1:i+gap])
    return torch.stack(A), torch.stack(B).squeeze(1)

def sum_pos_neg(aa, mode='min', gap=2):  # 若负文档数量超过1，则对其求平均值（或最小值）提取一个负文档出来
    A, B = [], []
    bs = aa.size(0)
    for i in range(0, bs, gap):
        A.append(aa[i])
        B.append(aa[i+1:i+gap])
    A, B = torch.stack(A), torch.stack(B)*-1.0
    if mode == 'average':
        B= torch.sum(B, dim=1)/B.size(1)
    else:
        B = torch.min(B, dim=1)[0]
    return A + B

def get_ot_distanse(query, doc):   # 封装好的提取ot距离，直接传入问题特征和文档特征就可以获取他们之间的ot距离
	cos_distance = cost_matrix_batch_torch(query.transpose(2, 1), doc.transpose(2, 1))
	cos_distance = cos_distance.transpose(1,2)  # 运输成本
	#修改点将0.1改为0.0
	beta = 0.5
	...
	ot_loss = - IPOT_distance_torch_batch_uniform(cos_dist, query.size(0), query.size(1), doc.size(1), 30)
	gw_loss = GW_distance_uniform(query.transpose(2,1), doc.transpose(2,1))
	# print(ot_loss, gw_loss)

	return 1. * torch.sum(gw_loss) + 1. * torch.sum(ot_loss)
