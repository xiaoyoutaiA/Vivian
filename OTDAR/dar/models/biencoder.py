...
from .OT_torch_ import split_pos_neg, get_ot_distanse
...
class BiEncoderNllLoss(object):
    def calc(
        ...
        """
        scores = self.get_scores(q_pooled_vectors, ctx_pooled_vectors)  # q_pooled_vectors: bs*768  ctx_pooled_vectors: (2*bs)*768
        if len(q_pooled_vectors.size()) > 1:
            q_num = q_pooled_vectors.size(0)
        ...
        doc_pos, doc_neg = split_pos_neg(ctx_vectors)
        eps = 2e-3
        ot_loss_pos = get_ot_distanse(q_vectors, doc_pos)  # pos ot_loss: 0.3, gw_loss: 0.1  total: 0.19
        # print('pos: ', ot_loss_pos)
        ot_loss_neg = get_ot_distanse(q_vectors, doc_neg) # neg ot_loss: 0.3, gw_loss: 0.1  total: 0.19
        # print('neg: ', ot_loss_pos)
        ot_loss = max(0., sum(ot_loss_pos-ot_loss_neg+eps))
        # print('base: ', loss)
        # sigma=1.0
        # loss = loss + sigma*ot_loss
        loss = loss + ot_loss
